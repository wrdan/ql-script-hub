...
def task_complete(user_token, task_id, task_name):
    ...
    try:
        response = requests.post('https://api.zhumanito.cn/api/task/complete', headers=headers,
        ...
        if response_json['code'] == 200 and response_json['msg'] == '成功':
            for task_status in response_json['data']['task']:
                if task_status['task_id'] == task_id and task_status['status']:
                    print(
                        f"任务: {task_name} 成功，剩余水: {response_json['data']['user']['water_num']}, 剩余阳光: {response_json['data']['user']['sun_num']}")
        else:
...
def water(user_token):
    ...
    try:
        ...
        if response_json['code'] == 200 and response_json['msg'] == '成功':
            print(
                f"浇水成功，剩余水: {response_json['data']['user']['water_num']}, 剩余阳光: {response_json['data']['user']['sun_num']}")
